...
# Convert data to int. Special handling for datetime object, 
# where the timestamp function should be applied first to the datetime in order to convert.
def to_int(func):
    def wrapper(data_to_int):
        # Check if the input is a datetime object
        if isinstance(data_to_int, datetime):
            data_to_int = data_to_int.timestamp()     
        # Convert the data to an integer representing the timestamp
        return func(int(data_to_int))
    return wrapper
...
class Stock:
    ...
    def __init__(self, symbol, stock_type, last_dividend, fixed_dividend, par_value):
        ...
        if stock_type == 'Common' and fixed_dividend != 0 :
            raise ValueError("Fixed dividend for Preferred stock must be a number between 0 and 1")
    ...
    def record_trade(self, timestamp, quantity, buy_sell, traded_price):
        ...
        # The headq is a min heap, where the smallest value priority is the root. 
        # We need the latest trade (closer to now) to have higher priority,
        # but ( Timestamp of now ) > ( Timestamp before 5 minutes ). 
        # In order to achieve the reverse order based on the timestamp,
        # the get_date_as_int result can be reversed so now, ( - Timestamp of now ) < ( - Timestamp before 5 minutes )
